...
- [ ] Day 5: Alert preferences UI — per-location base/radius/escort-positions editing already exists from Week 1's "Manage Search Locations" page (redesigned 2026-08-19); what's actually new here is **a channel toggle (email/SMS/both)** and the profile-wide `alerts_muted` toggle, editable from the Pilot Car dashboard. Cheap to build now, and it's the fix for the "how do I stop getting alerts" complaint that shows up in Pilot Car Loads' own FAQ.
