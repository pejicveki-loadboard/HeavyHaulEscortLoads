...
- [ ] Day 1–2: Scaffold Next.js app, connect Prisma to Postgres, define initial schema:
  ...
  - `PilotCarProfile` (user_id FK, company name, phone, home base city/state + lat/lng, alert radius miles, escort position preferences — multi-select, alerts_muted boolean, **created_at (this is "signed up" for admin-visibility purposes)**, subscription_status enum: `none | trialing | active | expired`, **trial_started_at — added 2026-08-19, set the moment the profile is created**, trial_ends_at, **paid_started_at — added 2026-08-19, set the moment `subscription_status` first transitions to `active`, null until then**, stripe_customer_id, stripe_subscription_id — Stripe columns can exist now even though Stripe wiring is Phase 2)
  ...
  - `LoadAlert` (load_id FK, pilot_car_profile_id FK, sent_at) — a simple send log so a load never emails the same profile twice (e.g., on repost or edit) and so the Load Manager dashboard's "X notifications sent" stat has something to count
...
- [ ] Day 5: Basic account/profile edit pages for both profile types
...
- [ ] Day 2–3: Load board browse page — **origin + radius search is must-have here, not deferred**: driver sets a home base + radius (e.g., 150 miles) and escort-position preferences in their profile, board filters/sorts loads by distance using stored lat/lng (a straight-line/haversine calculation in a plain SQL query is enough at launch scale — no need for PostGIS yet). Add date-range filtering alongside it. Every request through this endpoint re-checks `PilotCarProfile.subscription_status` server-side per the access control model above.
...
- [ ] **Day 1–2: Admin dashboard (added 2026-08-19, decision: build this now rather than defer it).** A set of pages under `/admin`, reachable only by accounts with `is_admin = true` (403 for everyone else, checked server-side — same rule as the subscription gate). Build:
  - **Summary view:** total Load Manager signups, total Pilot Car signups, total loads posted, and a breakdown of Pilot Car profiles by `subscription_status` (how many `trialing`, `active`, `expired`) — cheap counts against the tables that already exist from Week 1, no new schema needed beyond the `is_admin`/`trial_started_at`/`paid_started_at` fields added this week.
  - **Users table:** searchable/sortable list of every Load Manager and Pilot Car profile, showing company name, contact email/phone, **`created_at` ("signed up")**, and for Pilot Car profiles specifically: **`trial_started_at`, `trial_ends_at`, current `subscription_status`, and `paid_started_at`** (blank/— until they actually convert from trial to paid). This is the specific requirement from the 2026-08-19 planning conversation: Vedran needs to see exactly when someone signed up and when their 30-day free trial converted into an actual paying subscription, not just their current status.
