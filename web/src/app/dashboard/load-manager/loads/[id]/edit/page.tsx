...
export default async function EditLoadPage({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <Link href="/dashboard/load-manager" className="text-sm text-brand-accent underline transition-colors duration-150 hover:text-brand-accent-light active:text-brand-accent-deep">
          &larr; Back to dashboard
        </Link>
        <h1 className="mt-2 text-2xl font-semibold text-brand-text">Edit load</h1>
      </div>
      <LoadForm
        mode="edit"
        loadId={load.id}
